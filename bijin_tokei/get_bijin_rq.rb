	...
	0..24.times do |hour|
	   0..60.times do |minute|
	      ...
	      begin
	         ...
	         response = http.get(
	         	"/cc/tokei_images/#{now_hour}#{now_minute}.jpg",
	         	{"Referer" => "http://bijint.com/cc/" }
	         )
	         open( $scriptPath + getTimeNow(0) + '_rq/' + "#{now_hour}#{now_minute}.jpg", "wb" ) do |file|
	             file.puts response.body
	         end
	      end
	      rescue 
	         p "#{now_hour}#{now_minute}.jpg can not get"
	         error_files += "http://bijint.com/cc/tokei_images/#{now_hour}#{now_minute}.jpg\n"
	      end
	   end
	end
end
...
#
def init()
	if mkdir( $scriptPath + getTimeNow(0) + '_rq' )
		getBijin()
		p 'get bijin_tokei_rq !'
	end
end
